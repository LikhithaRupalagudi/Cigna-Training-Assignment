--1.List boks with exactly one available copy.
  SELECT * FROM  Books
  WHERE Available_Copies= 1;

--2.Lists authors whose names starts with a specific letter (eg J).
 SELECT Author_Name FROM  Authors
 WHERE Author_Name LIKE  'J%';

--3. List members with no address
...
WHERE Return_Date = TO_DATE('2025-10-10','YYYY-MM-DD');
